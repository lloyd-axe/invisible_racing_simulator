...
class HolisticModel:
    ...
    def _draw_landmarks(self, frame: Any, results: Any, other_points: list, lines: list) -> Any:
        ...
        for p in other_points:
            cv2.circle(frame, p, 5, (0, 0, 255), cv2.FILLED)
        for line in lines:
            if len(line) == 2:
...
class InvisibleRacingSim(HolisticModel):
    ...
    def non_letter_keys(self, key):
        return self.key_dict.get(key, key)

    def get_hand_points(self, results):
    ...
    def __steer(self, percent_turn, delta_angle, state_direction, prev_state_direction):
    ...
    def run(self):
        ...
        while self.capture.isOpened():
            ...
            if right_mid and left_mid:
                ...
                self.__steer(percent_turn, delta_angle, state_direction, prev_state_direction)
                self.__throttle(distance)

            prev_perc_turn = percent_turn
            prev_state_direction = state_direction

            cv2.imshow('Invisible Racing Sim', cv2.flip(frame, 1))
